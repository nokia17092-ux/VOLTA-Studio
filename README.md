...
Run the source: `python3 -m http.server` in this folder, then open http://localhost:8000
(`fetch` does not work over `file://`, so an HTTP server is required).
...
Запуск исходника: `python3 -m http.server` в этой папке → http://localhost:8000
(по `file://` fetch не работает — нужен http-сервер).
